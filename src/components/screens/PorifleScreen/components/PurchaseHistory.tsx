          ...
          <TableBody items={borrows?.results}>
            {(item) => (
              <TableRow key={item.id}>
                <TableCell>
                  {item.id.slice(0, 3)}...{item.id.slice(item.id.length - 3)}
                </TableCell>
                <TableCell>{item.book_id.title}</TableCell>
                ...
                <TableCell>
                  <CustomButton onClick={() => route.push(`/book/${item.book_id.slug}`)}>Xem sách</CustomButton>
                </TableCell>
              </TableRow>
            )}
          </TableBody>
